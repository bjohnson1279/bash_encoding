...
## 2024-11-21 - Replace External Call Subshells with Native Bash Substrings
**Learning:** Calling `$(basename "$1")` and parsing outputs via `sed` in a script or loop introduces immense overhead. Switching to native bash functions and string manipulations using `${var##*/}`, bash built-in parameter expansion `${var//./ }`, and regex matching `[[ "$var" =~ regex ]]` can speed up executions by over 100x. Additionally, avoid leaving dangling or temporary testing scripts in the repo after benchmarking.
**Action:** Always prefer native bash regex and string manipulation (parameter expansion, extglob) over subshells and external processes (`sed`, `cat`, `basename`, `dirname`, `tr`) when working with strings inside frequently called functions. Verify code formatting and expected outcomes strictly.
