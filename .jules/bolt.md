...
## 2024-05-25 - Avoid Nameref vs Printf Micro-Optimizations
**Learning:** Replacing bash namerefs (`local -n`) with `printf -v` for dynamic variable assignment is a negligible micro-optimization. In scripts dominated by heavy external processes (like `ffmpeg`), this provides no measurable real-world performance impact and risks introducing bugs or unnecessary code churn.
**Action:** Do not optimize variable assignment methods (like replacing `local -n` with `printf -v`) in shell scripts unless it occurs millions of times in a tight loop and is proven via benchmarking to be the primary bottleneck. Focus on optimizing slow process execution (subshells, external commands, regex engines) instead.
