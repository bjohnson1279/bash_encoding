...
@test "get_avail_mb calculates available MB correctly" {
    ...
    df() {
        ...
        printf '%s\n' "/dev/sda1          1000000 500000      2048      50% /mock/path"
    }
    export -f df
...
@test "folder_sync returns 1 when available space is less than required space" {
    ...
    get_avail_mb() {
        local out_ref="${2:-}"; if [ -n "$out_ref" ]; then printf -v "$out_ref" "%s" "500"; else printf '%s\n' "500"; fi
    }
...
@test "folder_sync returns 1 when source folder size is greater than available space" {
    ...
    get_avail_mb() {
        local out_ref="${2:-}"; if [ -n "$out_ref" ]; then printf -v "$out_ref" "%s" "1500"; else printf '%s\n' "1500"; fi
    }
    export -f get_avail_mb

    # Mock get_folder_size_mb to return 2000 (greater than 1500 available)
    get_folder_size_mb() {
        local out_ref="${2:-}"; if [ -n "$out_ref" ]; then printf -v "$out_ref" "%s" "2000"; else printf '%s\n' "2000"; fi
    }
...
@test "get_avail_mb fails safely on non-numeric injection" {
    df() {
        printf '%s\n' "Filesystem     1024-blocks   Used Available Capacity Mounted on"
        printf '%s\n' "/dev/sda1          1000000 500000      a[\$(echo 1 > $TEST_TEMP_DIR/hacked)]      50% /mock/path"
    }
    export -f df
...
@test "get_folder_size_mb fails safely on non-numeric injection" {
    du() {
        printf '%s\n' "a[\$(echo 1 > $TEST_TEMP_DIR/hacked)]	/mock/path"
    }
    export -f du
    rm -f -- "$TEST_TEMP_DIR/hacked"
    mkdir -p "$TEST_TEMP_DIR/mock_dir"
